...
# 测试集标签的位置
test_label_path = os.path.join('image129','ground.txt')
...
def train_and_test():
    ...
    result_list = []
    result_list1 = []
    test_names = glob.glob(os.path.join(test_feat_path, '*.feat'))
    test_names.sort(key=lambda l: int(re.findall('\d+', l)[0]))
 #   print(test_names)
 #   for feat_path in glob.glob(os.path.join(test_feat_path, '*.feat')):
    for feat_path in test_names:
        total += 1
        ...
        result_list1.append(label_map[int(result[0])] + '\n')
        if int(result[0]) == int(data_test[-1]):
            correct_number += 1
    write_to_txt(result_list)
    write_to_txt1(result_list1)
...
def write_to_txt1(list):
    with open('zhj.txt', 'w') as f:
        f.writelines(list)
    print('zhj的测试结果zhj.txt里面')
